...
vast::operation kernel_visitor::visit(const vast::cg::clang_stmt *stmt,
  ...
  auto name = expansion.spelling.m_name;
  if (KernelDialect::rcu_access_pointer.m_name == name ||
      KernelDialect::rcu_dereference.m_name == name ||
      KernelDialect::rcu_dereference_bh.m_name == name ||
      KernelDialect::rcu_dereference_sched.m_name == name) {
    auto p = m_view.visit(expansion.arguments[0], scope)->getResult(0);

    if (KernelDialect::rcu_access_pointer.m_name == name) {
      return m_bld.create<RCUAccessPointer>(loc, rty, p);
    } else if (KernelDialect::rcu_dereference.m_name == name) {
      return m_bld.create<RCUDereference>(loc, rty, p);
    } else if (KernelDialect::rcu_dereference_bh.m_name == name) {
      return m_bld.create<RCUDereferenceBH>(loc, rty, p);
    } else if (KernelDialect::rcu_dereference_sched.m_name == name) {
      return m_bld.create<RCUDereferenceSched>(loc, rty, p);
    }
  } else if (KernelDialect::rcu_assign_pointer.m_name == name) {
    auto p = m_view.visit(expansion.arguments[0], scope)->getResult(0);
    auto v = m_view.visit(expansion.arguments[1], scope)->getResult(0);
    return m_bld.create<RCUAssignPointer>(loc, rty, p, v);
  } else if (KernelDialect::rcu_replace_pointer.m_name == name) {
